...
def save_train_data(file_path,pattern ='origin'):
    ...
    torch.save(all_sequences,'embedding_data/test_sequences.pt')
    torch.save(context_mask,'embedding_data/test_context_mask.pt')
    torch.save(ans_start_mask,'embedding_data/test_ans_start_mask.pt')
    torch.save(ans_end_mask,'embedding_data/test_ans_end_mask.pt')
    torch.save(sentence_idx,'embedding_data/test_sentence_idx.pt')
    np.save('embedding_data/test_id2data.npy',id2data)
    #return all_sequences,all_inputs, att_mask, context_mask, ans_start_mask, ans_end_mask, sentence_idx, na_output, id2data

save_train_data('train.txt')